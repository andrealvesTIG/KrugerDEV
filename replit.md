...
- **Page**: `client/src/pages/PmoRadar.tsx` — fetches both risks (`/api/issues?itemType=risk`) and issues (`/api/issues?itemType=issue`), projects, and portfolios; transforms into radar signals. Issues use priority/severity for scoring (via `SEVERITY_MAP`/`PRIORITY_SCORE_MAP`), `targetResolutionDate` for due date, and are rendered as **square** dots on the radar (risks are circles). An "Item Type" filter dropdown in FiltersPanel allows showing All Items, Risks Only, or Issues Only. Signal IDs for issues are prefixed with `issue-` to distinguish from risk IDs
...
- **Overdue Visualization**: Signals with `timeOffsetDays < 0` (past the NOW line) are forced RED regardless of risk score, with full opacity, thicker stroke, and red glow. If `costExposure` is set, a compact formatted label (e.g. "$250K", "$1.5M") renders next to the dot. Tooltip shows "OVERDUE" badge (when dueDate exists), formatted cost exposure in red, and due date with human-readable time distance (e.g. "Mar 15, 2026 (Due in 15 days)", "Jan 10, 2026 (49 days overdue)", "Due in 2mo 5d", "1y 3mo overdue")
