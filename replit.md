...
**Data Model**: Supports Portfolios, Projects, hierarchical Tasks, Resources, Risks, Issues, Milestones, Change Requests, Project Documents, and Organizations. It includes multi-tenancy with soft-delete and role-based access control (Owner, Admin, Member, Team Member). **Milestones are consolidated into the tasks table** (with `isMilestone=true` and `taskType='Milestone'`). The standalone `milestones` table is deprecated — all milestone CRUD, queries, and portfolio views now read/write the `tasks` table. The storage layer provides a `taskToMilestone()` mapping function to preserve backward-compatible Milestone-shaped API responses.
