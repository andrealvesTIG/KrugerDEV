...
**API Design**: Follows a consistent API error convention and maintains a comprehensive OpenAPI 3.0 specification (`server/swagger.ts`) with 351 documented endpoint paths across 49 schemas and 27+ tags. Swagger spec includes: requiredness annotations for all parameters that return 400 when missing; pagination documentation for `/projects`; full coverage of Training, Approval Delegations, Rejection Templates, Timesheet Compliance/SLA, Dashboard extensions, User profile analytics, MPP/Dataverse imports, admin monitoring, Project Templates (CRUD + apply/preview/from-project), Referral system (codes, validation, stats, payouts), Timesheet extensions (audit-log, comments, reminders, bulk-approve, settings), Portfolio extensions (escalated-items, scoring-rollup, scoring-config), Project extensions (import-csv, make-editable, sync-planner, tasks-baseline, status-report, risks/pdf, share), Risk extensions (ai-mitigation, convert-to-issue), Org extensions (scheduling-defaults, full-task-assignments, issue-assignments), Planner import, and UNCON2026 selfie routes. Milestone and MilestoneRequest schemas are marked deprecated. Key schemas (Organization, Portfolio, Project, Task, ProjectTemplate) include example values.
...
**Code Organization**: Critical oversized files have been modularized, including `server/routes.ts`, `server/storage.ts`, `client/src/pages/SuperAdmin.tsx`, and `client/src/pages/OrgSettings.tsx`. Duplicate industry landing pages were consolidated, and Vite `manualChunks` are configured for vendor code splitting. Unused code cleanup removed 8 unused UI component files (aspect-ratio, carousel, context-menu, input-otp, menubar, navigation-menu, toggle-group, toggle), 1 orphaned component (ProjectFilterViewsDropdown), and cleaned unused imports from 13 major page/component files.
...
**Partner Program**: Public `/partners` landing page targeting PMO consulting firms, independent consultants, and trainers/educators. Features a partner application form with honeypot bot protection, three partner type cards, benefits section, community trust section (PMI, PMO GA, IPMA, APM, AIPM), and stats. Applications stored in `partner_applications` table via `POST /api/partner-applications`. "Partners" link added to navigation on all public marketing pages (LandingPageNew, IndustryLandingPage, UnCon2026LandingPage). Route: `server/routes/partnerRoutes.ts`, page: `client/src/pages/PartnerProgramPage.tsx`.
