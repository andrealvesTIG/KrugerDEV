...
**API Design**: Follows a consistent API error convention and maintains a comprehensive OpenAPI 3.0 specification (`server/swagger.ts`) with 312 documented endpoint paths across 47 tags. Swagger spec includes requiredness annotations for all parameters that return 400 when missing, pagination documentation for `/projects`, and full coverage of Training, Approval Delegations, Rejection Templates, Timesheet Compliance/SLA, Dashboard extensions, User profile analytics, MPP/Dataverse imports, and admin monitoring endpoints.
