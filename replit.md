...
**Gantt Excel-like Grid Editing**: Cell-level focus (blue ring), arrow key/Tab navigation, Shift+Click/Arrow range selection (light blue highlight), type-to-edit and Enter-to-edit, cell-aware copy (Ctrl+C copies focused cell, range, or selected rows) and paste (Ctrl+V starts at focused cell's row and column, with progress dialog and cancel support). Read-only columns (WBS, #, Critical, Summary, Est Hours) are skipped during navigation. Resources column is selectable/pasteable. Paste supports resource assignment (auto-match or create). Backward compatible with row-level checkbox copy/paste.
**Manual Task Scheduling**: Tasks support `schedulingMode` ('auto'|'manual'). Manually scheduled tasks can exist without start/end dates. Toggle via task action menu in Gantt view. Timeline shows "Manually Scheduled" badge for dateless manual tasks. Clearing date cells (via Delete/Backspace key or calendar Clear button) automatically switches task to manual scheduling mode.
