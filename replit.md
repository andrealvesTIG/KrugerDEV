...
*   **Enterprise Calendar (Phase 2 — wired into CPM/Gantt)**: `defaultLegacyResolvedCalendar()` in `shared/lib/calendarEngine.ts` provides the Mon–Fri 8h fallback used everywhere a calendar isn't supplied (preserves all existing scheduling behaviour). Calendar-aware overloads in `client/src/lib/workingDays.ts` (`isWorkingDayCal`, `addWorkingDaysCal`, `workingDaysBetweenCal`, `calculateEndDateFromWorkingDaysCal`, `calculateDurationInWorkingDaysCal`, `calculateStartDateFromEndAndDurationCal`) accept an optional `ResolvedCalendar` and walk day-by-day via the engine when supplied. `calculateCPM(tasks, deps, calendar?)` in `client/src/lib/cpm.ts` threads the calendar through forward + backward passes, all `getDurationDays` / `workingDaysFromProjectStart` / `dateFromWorkingDayOffset` helpers, and the date formatting on results. Endpoints `GET /api/projects/:projectId/resolved-calendar` and `GET /api/resources/:resourceId/resolved-calendar?projectId=` in `server/routes/calendarRoutes.ts` resolve project.calendarId → org default → null and (for resources, when projectId is supplied) layer the resource calendar's non-working windows on top via `withAdditionalNonWorkingWindows`. Storage helper `getDefaultCalendarForOrg(orgId)` in `server/storage/calendarStorage.ts`. Hook `client/src/hooks/use-resolved-calendar.ts` (`useProjectResolvedCalendar`, `useResourceResolvedCalendar`). `ProjectGanttView` now subscribes to the project's resolved calendar and passes it to `calculateCPM`. Tests in `tests/calendarAwareScheduling.test.ts`. **Slice C wired**: `/api/resources/:resourceId/resolved-calendar` now folds approved `resource_availability` rows (PTO, leave, sick, etc.) in as additional non-working windows via `withAdditionalNonWorkingWindows` in all three response paths (project+resource overlay, project-only/resource-only base + PTO, resource-only + PTO). Partial-day rows (`hoursPerDay` set) are still treated as full-day non-working until Phase 3 minute-precision lands. New storage helper `getResolvedCalendarForProject(projectId)` in `server/storage/calendarStorage.ts` (also on `IStorage`) resolves project.calendarId → org default → null. `propagateScheduleForProject(projectId)` in `server/routes/projectItemRoutes.ts` fetches the project's resolved calendar at function entry and threads it through `getConstraintFromDep` and the start/end recompute via `*Cal` helpers. Both manual-startDate-change lag-recompute (~line 2044) and dep-create constraint enforcement (~line 2304) now resolve the dependent task's project calendar and use `*Cal` helpers (`nextWorkingDayCal`, `ensureWorkingDayCal`, `addWorkingDaysCal`, `calculateEndDateCal`, `calculateDurationCal`, `workingDaysBetweenExclusiveCal`). Tests in `tests/calendarAwareScheduling.test.ts` cover PTO fold-in semantics (271 total). **Not yet wired**: Gantt drag/resize snap-to-working-time, CreateTaskDialog/IntakeDetails duration math, resource_availability → engine fold-in for the assignment-overlay scheduler, partial-day PTO honouring (Phase 3). Project calendar wins; resource calendar restricts availability only.
