...
- **Time Projection**: Slider in FiltersPanel (0-12 months) shifts all signal timeOffsetDays by `projectionOffsetDays = months * 30.44`, moving future risks toward/past center. Unresolved risks (not Closed/Mitigated) clamp at -85 days so they never scroll off the radar during simulation — only resolved risks disappear. Header shows projected date badge, RadarCanvas center label switches from "NOW" to projected month/year. Reset button returns to present
