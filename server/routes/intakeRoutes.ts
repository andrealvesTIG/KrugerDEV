...
export function registerIntakeRoutes(app: Express) {
  ...
  apiRoute(app, 'get', '/api/organizations/:orgId/intake-workflow', {
    ...
    try {
      ...
      // Validate workflow ownership if explicit
      if (workflowId) {
  ...
  apiRoute(app, 'post', '/api/organizations/:orgId/intake-workflows', {
    ...
    parameters: [pathId('orgId')],
    requestBody: body({ type: 'object', properties: { name: { type: 'string' }, description: { type: 'string' }, isDefault: { type: 'boolean' } }, required: ['name'] }),
  ...
  apiRoute(app, 'patch', '/api/organizations/:orgId/intake-workflows/:wfId', {
    tag: 'Intake Workflow',
    summary: 'Update an intake workflow (name, description, isDefault)',
