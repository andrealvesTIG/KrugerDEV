...
/**
 * Count the distinct resources visible in the Resources directory feed. Reading
 * other people's actuals depends on the connected admin holding the right
 * Project Online permission; when that permission is missing the reporting feed
 * typically returns only the signed-in user. Callers use this as an early
 * read-only signal: a count of 1 (or 0) means we can likely only see one
 * person, so the sync would silently import a single user's time unless we warn.
 * Distinct identity is keyed by UID, then email, then display name.
 */
export function countDistinctResources(rows: RawResourceRow[]): number {
  const seen = new Set<string>();
  for (const r of rows || []) {
    const key = str(r.ResourceUID) ?? str(r.ResourceEmailAddress) ?? str(r.ResourceName);
