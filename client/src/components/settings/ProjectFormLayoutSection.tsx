...
interface DraftItem { uid: string; itemType: "field" | "custom_field" | "block" | "label"; itemKey: string; width: "full" | "half" | "third"; displayName: string | null; }
...
export function ProjectFormLayoutSection({ organizationId }: { organizationId: number }) {
  ...
  // Compute broken refs from the draft so admins can see (and clean up) layout
  // items that point to a deleted field/block/custom-field.
    ...
    for (const t of draft) {
      for (const s of t.sections) {
        for (const i of s.items) {
          let ok = false;
          if (i.itemType === "field") ok = !!PROJECT_FORM_FIELDS.find(f => f.key === i.itemKey);
          else if (i.itemType === "block") ok = !!PROJECT_FORM_BLOCKS.find(b => b.key === i.itemKey);
          else if (i.itemType === "label") ok = true;
          else if (i.itemType === "custom_field") ok = !!projectCustomFieldDefs.find(d => String(d.id) === i.itemKey);
          if (!ok) {
  ...
  const addItem = (tabUid: string, secUid: string, itemType: DraftItem["itemType"], itemKey: string) => {
    ...
    updateSection(tabUid, secUid, {
      items: [...(tab.sections.find(s => s.uid === secUid)?.items ?? []), { uid: uid("itm"), itemType, itemKey, width: "full", displayName: defaultDisplayName }],
    });
  };
      ...
      <CardContent className="space-y-4">
        {brokenRefs.length > 0 && (
          <div
            ...
            data-testid="project-form-broken-refs"
          >
