...
export function ProjectSingleCustomField({
  ...
  const renderValue = () => {
    ...
    if (field.fieldType === "rag_rollup") {
      ...
      if (!worst) {
        return (
          <span className="text-muted-foreground text-sm inline-flex items-center gap-1" data-testid={`value-project-rag-rollup-empty-${field.id}`}>
            —
            <Tooltip>
              <TooltipTrigger asChild>
      ...
      return (
        <span className="inline-flex items-center gap-1.5" data-testid={`value-project-rag-rollup-${field.id}`}>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button type="button" className="text-muted-foreground hover:text-foreground" data-testid={`tooltip-project-rag-rollup-${field.id}`}>
                <Info className="h-3.5 w-3.5" />
              </button>
            </TooltipTrigger>
            <TooltipContent className="text-xs">{tooltipText}</TooltipContent>
    ...
    if (field.fieldType === "threshold_check") {
      ...
      if (!cfg) {
        return (
          <span className="text-muted-foreground text-sm italic" data-testid={`value-project-threshold-misconfig-${field.id}`}>
            Not configured
          </span>
        );
      }
      ...
      if (sourceNum == null) {
        return (
          <span className="text-muted-foreground text-sm inline-flex items-center gap-1" data-testid={`value-project-threshold-empty-${field.id}`}>
            —
            <Tooltip>
              <TooltipTrigger asChild>
                <button type="button" className="text-muted-foreground hover:text-foreground">
      ...
      const cls = passed
        ...
        : "bg-red-100 text-red-800 border-red-300 dark:bg-red-900/40 dark:text-red-200";
      return (
        <span className="inline-flex items-center gap-1.5" data-testid={`value-project-threshold-${field.id}`}>
          <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${cls}`}>
            {passed ? "Pass" : "Fail"}
          </span>
          <Tooltip>
            <TooltipTrigger asChild>
              <button type="button" className="text-muted-foreground hover:text-foreground" data-testid={`tooltip-project-threshold-${field.id}`}>
                <Info className="h-3.5 w-3.5" />
              </button>
            </TooltipTrigger>
    ...
    if (field.fieldType === "formula") {
      ...
      if (!expr) {
        return (
          <span className="text-muted-foreground text-sm italic" data-testid={`value-project-formula-misconfig-${field.id}`}>
            Not configured
          </span>
        );
      }
      ...
      if (!result.ok) {
        return (
          <span className="text-destructive text-xs inline-flex items-center gap-1" data-testid={`value-project-formula-error-${field.id}`}>
            Error
            <Tooltip>
              <TooltipTrigger asChild>
      ...
      return (
        <span className="text-sm inline-flex items-center gap-1" data-testid={`value-project-formula-${field.id}`}>
          <span className="font-medium">{display}</span>
          <Tooltip>
            <TooltipTrigger asChild>
              <button type="button" className="text-muted-foreground hover:text-foreground" data-testid={`tooltip-project-formula-${field.id}`}>
                <Info className="h-3.5 w-3.5" />
              </button>
            </TooltipTrigger>
