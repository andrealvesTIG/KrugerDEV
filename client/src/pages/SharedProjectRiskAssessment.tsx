...
export default function SharedProjectRiskAssessment() {
  const [, params] = useRoute("/project-risk-assessment/share/:token");
  ...
  const { data, isLoading, error } = useQuery<{
    ...
    queryKey: ["/api/project-risk-assessments/share", token],
    enabled: !!token,
  });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2" data-testid="text-shared-project-name">
            <Shield className="h-6 w-6" />
            {data.projectName} — Project Risk Assessment
          </h1>
        ...
        <Button
          variant="outline"
          onClick={() => {
            window.open(`/api/project-risk-assessments/share/${token}/pdf`, "_blank");
          }}
